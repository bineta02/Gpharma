                    ...
                    <tr>
                      ...
                      <td class="text-center">
                        <div class="d-flex justify-content-center gap-1">
                          <a href="{{ route('achats.show', $achat->id) }}" class="btn btn-info btn-sm text-white" title="Voir">
                            <i class="bi bi-eye"></i>
                          </a>
                          ...
                          @if($achat->statut == 'en_attente')
                            <form action="{{ route('achats.receptionner', $achat->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('PATCH')
                              <button type="submit" class="btn btn-success btn-sm" title="Réceptionner">
                          ...
                          @if($achat->statut != 'annule')
                            <form action="{{ route('achats.annuler', $achat->id) }}" method="POST" onsubmit="return confirm('Annuler cet achat ?')" style="display:inline;">
                              @csrf
                              @method('PATCH')
